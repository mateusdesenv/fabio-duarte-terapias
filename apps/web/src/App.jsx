...
const phoneDigits = (value = '') => { const digits = String(value).replace(/\D/g, ''); return (digits.startsWith('55') ? digits : `55${digits}`).slice(0, 13) }
const formatPhone = (value = '') => { const digits = phoneDigits(value); const area = digits.slice(2,4); const first = digits.slice(4,9); const last = digits.slice(9,13); return `+55${area?` (${area}${area.length===2?')':''}`:''}${first?` ${first}`:''}${last?`-${last}`:''}` }
...
function AppointmentModal({ data, appointment, onClose, onSave }) {
  ...
  const [packageForm,setPackageForm]=useState({name:'Pacote personalizado',interval:7,total:'',items:firstService?[{serviceId:firstService.id,quantity:1}]:[]})
  ...
  const selectedServices=packageForm.items.map(item=>({...item,service:data.services.find(service=>service.id===item.serviceId)})).filter(item=>item.service)
  const sessionCount=selectedServices.reduce((sum,item)=>sum+Number(item.quantity||0),0)
  const suggestedTotal=selectedServices.reduce((sum,item)=>sum+item.service.price*Number(item.quantity||0),0)
  const packageTotal=packageForm.total===''?suggestedTotal:Number(packageForm.total)
  const togglePackageService=(serviceId)=>setPackageForm(current=>({...current,items:current.items.some(item=>item.serviceId===serviceId)?current.items.filter(item=>item.serviceId!==serviceId):[...current.items,{serviceId,quantity:1}]}))
  const changeQuantity=(serviceId,quantity)=>setPackageForm(current=>({...current,items:current.items.map(item=>item.serviceId===serviceId?{...item,quantity:Math.max(1,Number(quantity))}:item)}))
  const buildPackage=()=>{
    const packageId=crypto.randomUUID()
    const sessions=selectedServices.flatMap(item=>Array.from({length:Number(item.quantity)},()=>item.service))
    const weights=sessions.map(service=>Number(service.price||0)); const weightTotal=weights.reduce((sum,value)=>sum+value,0)||sessions.length
    const totalCents=Math.round(packageTotal*100); let allocated=0
    return sessions.map((service,index)=>{const cents=index===sessions.length-1?totalCents-allocated:Math.round(totalCents*(weights[index]||1)/weightTotal);allocated+=cents;return {id:crypto.randomUUID(),packageId,packageName:packageForm.name||'Pacote personalizado',packageTotal,packageSession:index+1,packageSessions:sessions.length,patientId:form.patientId,serviceId:service.id,service:service.name,date:addDays(form.date,index*Number(packageForm.interval||0)),time:form.time,duration:Number(service.duration),price:cents/100,status:form.status,notes:form.notes}})
  }
  const submit=(event)=>{event.preventDefault();if(mode==='package'&&!appointment){if(!sessionCount)return alert('Selecione pelo menos um serviço.');onSave(buildPackage());return}onSave({ ...form, id: form.id || crypto.randomUUID(), duration: Number(form.duration), price: Number(form.price) })}
  return <div className="modal-layer" role="dialog" aria-modal="true"><form className={`modal ${mode==='package'?'package-modal':''}`} onSubmit={submit}>
    ...
    {!appointment&&<div className="booking-mode"><button type="button" className={mode==='single'?'active':''} onClick={()=>setMode('single')}><CalendarDays/>Consulta única</button><button type="button" className={mode==='package'?'active':''} onClick={()=>setMode('package')}><BriefcaseBusiness/>Criar pacote</button></div>}
    <div className="form-grid"><label className="span-2">Paciente<select {...field('patientId')}>{data.patients.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>{mode==='single'||appointment?<><label className="span-2">Serviço<select value={form.serviceId} onChange={e=>chooseService(e.target.value)}>{data.services.filter(s=>s.active || s.id===form.serviceId).map(s => <option value={s.id} key={s.id}>{s.name} · {s.duration} min · {formatCurrency(s.price)}</option>)}</select></label><label>Data<input type="date" {...field('date')} /></label><label>Horário inicial<input type="time" {...field('time')} /></label><label>Duração deste atendimento<input type="number" min="15" step="15" {...field('duration')} /><small className="field-hint">Término previsto: {endTime(form.time, form.duration)}</small></label><label>Valor deste atendimento<input type="number" min="0" step="0.01" {...field('price')} /><small className="field-hint">Pode ser alterado para aplicar desconto.</small></label></>:<><label className="span-2">Nome do pacote<input value={packageForm.name} onChange={e=>setPackageForm({...packageForm,name:e.target.value})}/></label><div className="span-2 package-services"><span>Serviços e quantidade de sessões</span>{data.services.filter(service=>service.active).map(service=>{const item=packageForm.items.find(selected=>selected.serviceId===service.id);return <div className={item?'selected':''} key={service.id}><label className="check"><input type="checkbox" checked={Boolean(item)} onChange={()=>togglePackageService(service.id)}/><span><strong>{service.name}</strong><small>{service.duration} min · {formatCurrency(service.price)}</small></span></label>{item&&<label className="quantity">Sessões<input type="number" min="1" max="50" value={item.quantity} onChange={e=>changeQuantity(service.id,e.target.value)}/></label>}</div>})}</div><label>Primeira sessão<input type="date" {...field('date')} /></label><label>Horário padrão<input type="time" {...field('time')} /></label><label>Intervalo entre sessões<input type="number" min="0" step="1" value={packageForm.interval} onChange={e=>setPackageForm({...packageForm,interval:e.target.value})}/><small className="field-hint">Em dias. Use 7 para sessões semanais.</small></label><label>Valor personalizado do pacote<input type="number" min="0" step="0.01" value={packageForm.total} placeholder={String(suggestedTotal)} onChange={e=>setPackageForm({...packageForm,total:e.target.value})}/><small className="field-hint">Sugestão: {formatCurrency(suggestedTotal)}. O valor será distribuído entre as sessões.</small></label><div className="span-2 package-summary"><div><span>Sessões</span><strong>{sessionCount}</strong></div><div><span>Valor avulso</span><strong>{formatCurrency(suggestedTotal)}</strong></div><div><span>Condição personalizada</span><strong>{formatCurrency(packageTotal)}</strong></div>{suggestedTotal>packageTotal&&<small>Desconto total de {formatCurrency(suggestedTotal-packageTotal)}</small>}</div></>}<label className="span-2">Status<select {...field('status')}>{Object.entries(statusMap).map(([id,s]) => <option key={id} value={id}>{s.label}</option>)}</select></label><label className="span-2">Observações<textarea rows="3" {...field('notes')} /></label></div>
    <div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" type="submit">{mode==='package'&&!appointment?`Salvar pacote com ${sessionCount} sessões`:'Salvar agendamento'}</button></div>
  </form></div>
}
...
function Agenda({ data, onEdit, openNew, setData }) {
  ...
  return <>
    ...
    <div className="agenda-layout"><div className="calendar-wrap" aria-label="Agenda semanal com horários de 00:00 a 23:00"><div className="calendar-grid"><div className="calendar-corner" />{days.map((d,i)=><div key={d} className={`day-head ${d===todayISO?'today':''}`}><span>{['Seg','Ter','Qua','Qui','Sex','Sáb'][i]}</span><strong>{d.slice(-2)}</strong></div>)}
      {hours.map(time => {const hour=Number(time.slice(0,2));const isNight=hour<6||hour>=18;return <div className="calendar-row" key={time}><div className={`time-label ${isNight?'night-hours':''}`}>{time}</div>{days.map(day => <div className={`calendar-cell ${isNight?'night-hours':''}`} key={day}>{data.appointments.filter(a=>a.date===day && a.time.slice(0,2)===time.slice(0,2)).map(a=>{const p=getPatient(data,a.patientId);return <button key={a.id} aria-label={`${p?.name}, ${a.time} até ${endTime(a.time,a.duration)}, ${a.service}`} className={`calendar-event ${a.status} ${selectedId===a.id?'selected':''}`} onClick={()=>setSelectedId(a.id)}><strong>{p?.name}</strong><span>{a.time}–{endTime(a.time,a.duration)}</span><span>{a.service}</span></button>})}</div>)}</div>})}</div></div>
      <aside className="detail-panel">{selected ? <><p className="eyebrow">DETALHES DA CONSULTA</p><div className="detail-person"><div className="avatar large">{patient?.name.split(' ').map(n=>n[0]).slice(0,2).join('')}</div><div><h2>{patient?.name}</h2><span>{formatPhone(patient?.phone)}</span></div></div>{selected.packageId&&<div className="package-badge"><BriefcaseBusiness/><div><strong>{selected.packageName}</strong><small>Sessão {selected.packageSession} de {selected.packageSessions} · pacote {formatCurrency(selected.packageTotal)}</small></div></div>}<div className="detail-info"><p><CalendarDays /> {formatDate(selected.date)}, {selected.time}–{endTime(selected.time, selected.duration)}</p><p><Clock3 /> {selected.duration} minutos</p><p><Sparkles /> {selected.service}</p><p><strong>{formatCurrency(selected.price)}</strong></p><Status value={selected.status} />{selected.notes && <div className="note">{selected.notes}</div>}</div><WhatsAppButton patient={patient} appointment={selected} /><button className="secondary-button wide" onClick={()=>onEdit(selected)}><Pencil /> Editar</button><button className="danger-button wide" onClick={deleteSelected}><Trash2 /> Excluir consulta</button></> : <div className="empty-state"><CalendarDays /><h3>Selecione uma consulta</h3><p>Os detalhes aparecerão aqui.</p></div>}</aside>
    </div>
  </>
}

function Patients({ data, setData }) {
  const empty={name:'',phone:'55',email:'',notes:''}
  ...
  const open=(patient=null)=>{setEditing(patient?.id||'new');setForm(patient?{...patient,phone:phoneDigits(patient.phone)}:empty)}
  const close=()=>{setEditing(null);setForm(empty)}
  const save=(e)=>{e.preventDefault();const phone=phoneDigits(form.phone);if(phone.length!==13)return alert('Informe o DDD da região e o número completo com 9 dígitos.');const patient={...form,phone,id:editing==='new'?crypto.randomUUID():editing};setData(d=>({...d,patients:editing==='new'?[...d.patients,patient]:d.patients.map(item=>item.id===editing?patient:item)}));close()}
  return <><PageHeader eyebrow="RELACIONAMENTO" title="Pacientes" subtitle="Cadastros e histórico de atendimentos." action={<button className="primary-button" onClick={()=>open()}><Plus /> Novo paciente</button>} />
    <div className="search-box"><Search /><input placeholder="Buscar paciente..." value={search} onChange={e=>setSearch(e.target.value)} /></div>
    <div className="patient-grid">{filtered.map(p=>{const count=data.appointments.filter(a=>a.patientId===p.id).length;return <article className="patient-card" key={p.id}><div className="patient-top"><div className="avatar large">{p.name.split(' ').map(n=>n[0]).slice(0,2).join('')}</div><div><h3>{p.name}</h3><span>{formatPhone(p.phone)}</span></div></div><p>{p.email}</p><div className="patient-meta"><span>{count} atendimentos</span><div className="patient-actions"><a href={`https://wa.me/${phoneDigits(p.phone)}`} target="_blank" rel="noreferrer"><MessageCircle /> WhatsApp</a><button onClick={()=>open(p)}><Pencil/> Editar</button></div></div>{p.notes&&<small className="patient-note">{p.notes}</small>}</article>})}</div>
    {editing&&<div className="modal-layer" role="dialog" aria-modal="true"><form className="modal small" onSubmit={save}><div className="modal-header"><div><p className="eyebrow">CADASTRO</p><h2>{editing==='new'?'Novo paciente':'Editar paciente'}</h2></div><button type="button" onClick={close}><X /></button></div><div className="form-grid"><label className="span-2">Nome<input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label className="span-2">Telefone com DDD<input type="tel" inputMode="numeric" maxLength="19" required value={formatPhone(form.phone)} onChange={e=>setForm({...form,phone:phoneDigits(e.target.value)})}/><small className={`field-hint ${phoneDigits(form.phone).length===13?'valid-phone':''}`}>{phoneDigits(form.phone).length===13?'Número completo':'Preencha o DDD da região e os 9 dígitos do número.'}</small></label><label className="span-2">E-mail<input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label className="span-2">Observações<textarea rows="3" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></label></div><div className="modal-actions"><button type="button" className="secondary-button" onClick={close}>Cancelar</button><button className="primary-button">{editing==='new'?'Salvar paciente':'Salvar alterações'}</button></div></form></div>}
  </>
}
...
function Appointments({ data, setData, onEdit, openNew }) {
  ...
  return <><PageHeader eyebrow="ACOMPANHAMENTO" title="Atendimentos" subtitle="Consulte e atualize o histórico das consultas." action={<button className="primary-button" onClick={openNew}><Plus /> Nova consulta</button>} />
    ...
    <div className="table-wrap"><table><thead><tr><th>Paciente</th><th>Data e período</th><th>Serviço</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>{list.map(a=>{const p=getPatient(data,a.patientId);return <tr key={a.id}><td><strong>{p?.name}</strong><small>{formatPhone(p?.phone)}</small></td><td>{formatDate(a.date)}<small>{a.time}–{endTime(a.time,a.duration)} · {a.duration} min</small></td><td>{a.service}{a.packageId&&<small>{a.packageName} · {a.packageSession}/{a.packageSessions}</small>}</td><td><strong>{formatCurrency(a.price)}</strong></td><td><select className={`status-select ${a.status}`} value={a.status} onChange={e=>updateStatus(a.id,e.target.value)}>{Object.entries(statusMap).map(([k,v])=><option value={k} key={k}>{v.label}</option>)}</select></td><td><div className="table-actions"><WhatsAppButton patient={p} appointment={a} compact/><button onClick={()=>onEdit(a)}><Pencil /></button></div></td></tr>})}</tbody></table></div>
  </>
}
...
export default function App() {
  ...
  const saveAppointment=(item)=>{setData(d=>{const items=Array.isArray(item)?item:[item];if(Array.isArray(item))return {...d,appointments:[...d.appointments,...items]};const current=items[0];return {...d,appointments:d.appointments.some(a=>a.id===current.id)?d.appointments.map(a=>a.id===current.id?current:a):[...d.appointments,current]}});setModal(null)}
